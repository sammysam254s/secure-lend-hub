...
Deno.serve(async (req) => {
  ...
  const body = `
    ...
    <div style="padding:16px 20px;border-bottom:1px solid #eee;">
      ...
      <div style="display:flex;justify-content:space-between;padding:6px 0;font-size:13px;"><span style="color:#666;">Created</span><span style="font-weight:600;">${new Date(contract.created_at).toLocaleDateString("en-KE")}</span></div>
      <div style="display:flex;justify-content:space-between;padding:6px 0;font-size:13px;"><span style="color:#666;">Due Date</span><span style="font-weight:600;">${new Date(contract.due_date).toLocaleDateString("en-KE", { dateStyle: "long" })}</span></div>
    </div>
  ...
  const html = buildPage("Contract Verified", body, false);
